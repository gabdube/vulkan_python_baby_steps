...
from ctypes import byref, pointer, c_uint32
...
def execute_commands(api, cmd, sub_buffers):
    ...
    api.CmdExecuteCommands(cmd, sub_buffer_count, sub_buffers_ptr) 
...
def queue_submit(api, queue, submit_info, fence=vk.Fence(0)):
    ...
    if result != vk.SUCCESS:
        raise RuntimeError("Queue submit info failed: {}".format(result))
